#include "IEEE802154MacAttacker.h"
#include "MICSec.h"
...
Define_Module(IEEE802154MacAttacker);

void IEEE802154MacAttacker::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        if (myMacAddr.isUnspecified())
        {
            ...
            else
            {
                ...
                macEV
                << "myMacAddr is unspecified -> assigning user set address (macAddr in omnetpp.ini) = " << myMacAddr << endl;
            }
        }
...
void IEEE802154MacAttacker::receiveChangeNotification(int category, const cPolymorphic *details)
{
    Enter_Method_Silent
    ();
...
void IEEE802154MacAttacker::setDefMpib()
{
    mpib = MacPIB();
...
void IEEE802154MacAttacker::handleMessage(cMessage *msg)
{
    if (ev.isGUI())
    {
        char bufTT[50];
        sprintf(bufTT, "isCoordinator: %s | isAssociated: %s", isCoordinator ? "true" : "false",
...
void IEEE802154MacAttacker::handleUpperMsg(cMessage *msg)
{
    ...
    {
        macEV
        << "A " << msg->getName() << " (#" << numUpperPkt << ") received from upper layer, but dropped due to concurrent msg processing \n";
        numUpperPktLost++;
        delete msg;
    ...
    if (msg->arrivedOn("MCPS_SAP"))
    {
        ...
        if (mpib.getMacSecurityEnabled())
        {
            ...
            drawBattery(IEEE802154MacAttacker::DATA_TEMP);
        ...
        switch (mappedMlmeRequestTypes[msg->getName()])
        {
            case MLMEASSOCIATE: {
                ...
                else if (dynamic_cast<AssociationRequest *>(msg))
                {
                    ...
                    if (mpib.getMacSecurityEnabled())
                    {
                        ...
                        drawBattery(IEEE802154MacAttacker::ASSOREQ_TEMP);
            ...
            case MLMEDISASSOCIATE: {
                ...
                if (disAss->getDevicePANId() == mpib.getMacPANId())
                {
                    ...
                    {
                        if (((disAss->getDeviceAddrMode() == addrLong) && (disAss->getDeviceAddress() == mpib.getMacCoordExtendedAddress())) || ((disAss->getDeviceAddrMode() == addrShort) && disAss->getDeviceAddress().getShortAddr() == mpib.getMacCoordShortAddress()))
                        {
...
void IEEE802154MacAttacker::handleUpperMLMEMsg(cMessage* msg)
{
    error("continue here to refactor everything!");
}

// handle messages received from UpperLayer via MCPS-SAP
void IEEE802154MacAttacker::handleUpperMCPSMsg(cMessage* msg)
{
    error("continue here to refactor everything!");
}

// handle messages received from PHY via PLME-SAP
void IEEE802154MacAttacker::handleLowerPLMEMsg(cMessage* msg)
{
...
void IEEE802154MacAttacker::handleLowerPDMsg(cMessage* msg)
{
...
void IEEE802154MacAttacker::handleSelfMsg(cMessage* msg)
{
    switch (msg->getKind())
    {
        case MAC_SCAN_TIMER:
...
void IEEE802154MacAttacker::handleBeacon(mpdu *frame)
{
    ...
    else
    {
        ...
        if (mpib.getMacSecurityEnabled())
        {
            ...
            drawBattery(IEEE802154MacAttacker::BEACON_TEMP);
        }
        else
        {
...
void IEEE802154MacAttacker::handleAck(cMessage* frame)
{
    ...
    if (mpib.getMacSecurityEnabled())
    {
        ...
        drawBattery(IEEE802154MacAttacker::ACK_TEMP);
    ...
    {
        macEV
        << "ACK #" << (unsigned int) ack->getSqnr() << " does not match a sequence number of a packet to transmit --> drop the ACK \n";
    }
    else if (waitDataAck == false && waitGTSAck == false)
    {
        macEV
        << "ACK #" << (unsigned int) ack->getSqnr() << " received while node is not waiting for an ACK --> drop the ACK \n";
    }

    return;
}

void IEEE802154MacAttacker::handleData(mpdu* frame)
{
    unsigned short frmCtrl = frame->getFcf();
...
void IEEE802154MacAttacker::handleCommand(mpdu* frame)
{
            ...
            if (isCoordinator)
            {
                ...
                if (mpib.getMacSecurityEnabled())
                {
                    ...
                    else if (tmpAssoReq->getAsh().secu.Seculevel == 4 || tmpAssoReq->getAsh().secu.Seculevel == 5 || tmpAssoReq->getAsh().secu.Seculevel == 6 || tmpAssoReq->getAsh().secu.Seculevel == 7)
                    {
                        ...
                        DevCap[1][0] == '\x00' ? false : true, //ffd
                        tmpAssoReq->getSrc(), //mac address ext
                        &DevCap[2][0], //pwrsrc
        ...
        case Ieee802154_ASSOCIATION_RESPONSE: {
            ...
            else
            {
                ...
                if (mpib.getMacSecurityEnabled())
                {
                    ...
                    drawBattery(IEEE802154MacAttacker::ASSORESP_TEMP);
...
void IEEE802154MacAttacker::handle_PLME_SET_TRX_STATE_confirm(phyState status)
{
    macEV
    << "PLME_SET_TRX_STATE_confirm with phyState " << phyStateToString(status) << " received from PHY, the requested state is " << phyStateToString(trx_state_req) << endl;
    simtime_t delay;
...
void IEEE802154MacAttacker::handle_PLME_CCA_confirm(phyState status)
{
...
void IEEE802154MacAttacker::handle_PLME_GET_confirm(cMessage* msg)
{
    GetConfirm* getConf = check_and_cast<GetConfirm *>(msg);

    macEV
    << "PLME-GET.confirm for attribute '" << PhyPIB_AttributesToString(PhyPIB_Attributes(getConf->getPIBattr())) << "' received from PHY with status: " << PhyPIB_StatusToString(PhyPIB_Status(getConf->getStatus())) << " and value: " << (double) getConf->getValue() << endl;
...
void IEEE802154MacAttacker::handle_PLME_SET_confirm(cMessage* msg)
{
    SetConfirm* setConf = check_and_cast<SetConfirm *>(msg);

    macEV
    << "PLME-SET.confirm for attribute '" << PhyPIB_AttributesToString(PhyPIB_Attributes(setConf->getPIBattr())) << "' received from PHY with status: " << PhyPIB_StatusToString(PhyPIB_Status(setConf->getStatus())) << endl;
...
void IEEE802154MacAttacker::handle_PD_DATA_confirm(phyState status)
{
    inTransmission = false;
...
bool IEEE802154MacAttacker::filter(mpdu* pdu)
{
...
void IEEE802154MacAttacker::sendMCPSDataConf(MACenum status, unsigned char msdu)
{
...
void IEEE802154MacAttacker::sendMCPSDataIndication(mpdu* rxData)
{
...
mpdu* IEEE802154MacAttacker::findRxMsg(MACAddressExt dest)
{
...
unsigned short IEEE802154MacAttacker::genFCF(frameType ft, bool secu, bool fp, bool arequ, bool pid, AddrMode dam, unsigned short fv, AddrMode sam)
{
...
void IEEE802154MacAttacker::genSetTrxState(phyState state)
{
...
void IEEE802154MacAttacker::genMLMEResetConf()
{
...
void IEEE802154MacAttacker::genCCARequest()
{
...
void IEEE802154MacAttacker::genACK(unsigned char dsn, bool fp)
{
...
void IEEE802154MacAttacker::genMLMEDisasConf(MACenum status)
{
...
void IEEE802154MacAttacker::genAssoResp(MlmeAssociationStatus status, AssoCmdreq* tmpAssoReq)
{
    ...
    macEV
    << "Generating MLME-ASSOCIATE.response for: " << devAddr << " with short address: " << devAddr.getShortAddr() << " and #seqNr: " << (int) assoResp->getSqnr() << endl;
...
void IEEE802154MacAttacker::genDisAssoCmd(DisAssociation* disAss, bool direct)
{
...
unsigned short IEEE802154MacAttacker::calcFCS(mpdu* pdu, cMessage *payload, bool calcFlag, int headerSize)
{
...
void IEEE802154MacAttacker::genBeaconInd(mpdu* frame)
{
...
void IEEE802154MacAttacker::genGTSConf(GTSDescriptor gts, MACenum status)
{
...
void IEEE802154MacAttacker::genScanConf(ScanStatus status)
{
    ScanConfirm* scanConf = new ScanConfirm("MLME-SCAN.confirm");
...
void IEEE802154MacAttacker::genPollConf(MACenum status)
{
...
void IEEE802154MacAttacker::genStartConf(MACenum status)
{
...
void IEEE802154MacAttacker::genOrphanInd()
{
...
void IEEE802154MacAttacker::reqMsgBuffer()
{
...
void IEEE802154MacAttacker::sendDown(mpdu* frame)
{
...
void IEEE802154MacAttacker::doScan()
{
    switch (scanType)
...
double IEEE802154MacAttacker::getRate(char bitOrSymbol)
{
    double rate = 0;
...
void IEEE802154MacAttacker::getRadioChannel()
{
...
void IEEE802154MacAttacker::setRadioChannel(unsigned short newRadioChannel)
{
...
void IEEE802154MacAttacker::getCurrentChannelPage()
{
...
void IEEE802154MacAttacker::setCurrentChannelPage(unsigned int newChannelPage)
{
...
void IEEE802154MacAttacker::getSHRDuration()
{
...
void IEEE802154MacAttacker::getSymbolsPerOctet()
{
...
//-------------------------------------------------------------------------------/
void IEEE802154MacAttacker::csmacaEntry(char pktType)
{
...
void IEEE802154MacAttacker::csmacaResume()
{
...
void IEEE802154MacAttacker::csmacaStart(bool firsttime, mpdu* frame, bool ackReq)
{
    if (txAck)
    {
        backoffStatus = 0;
    ...
    {
        macEV
        << "[CSMA]: Starting CSMA-CA for " << frame->getName() << ":#" << (unsigned short) frame->getSqnr() << endl;
...
void IEEE802154MacAttacker::csmacaCancel()
{
...
void IEEE802154MacAttacker::csmacaCallBack(phyState status)
{
...
void IEEE802154MacAttacker::csmacaReset(bool bcnEnabled)
{
    if (bcnEnabled)
    {
        NB = 0;
...
simtime_t IEEE802154MacAttacker::csmacaAdjustTime(simtime_t wtime)
{
...
simtime_t IEEE802154MacAttacker::csmacaLocateBoundary(bool toParent, simtime_t wtime)
{
    int align;
...
bool IEEE802154MacAttacker::csmacaCanProceed(simtime_t wtime, bool afterCCA)
{
    ...
    if (!toParent(tmpCsmaca))  // as a coordinator
    {
        if (mpib.getMacBeaconOrder() != 15)  // beacon enabled as a coordinator
        {
            ...
            else  // Check 2
            {
                ...
                macEV
                << "[CSMA]: wtime = " << wtime << "; mpib.getMacBeaconTxTime = " << mpib.getMacBeaconTxTime() << endl;
                tmpf = tmpf / bPeriod;
    ...
    else  // as a device
    {
        if (rxBO != 15)  // beacon enabled as a device
        {
            ...
            else
            {
                ...
                macEV
                << "[CSMA]: t_bPeriods = " << t_bPeriods << "; rxBcnDuration = " << (unsigned short) rxBcnDuration << endl;
...
simtime_t IEEE802154MacAttacker::getFinalCAP(char trxType)
{
    ...
    else    // beacon enabled
    {
        ...
        if (trxType == 't')  // check TX CAP
        {
            if (mpib.getMacBeaconOrder() != 15)  // beacon enabled
            {
                ...
                else
                {
                    ...
                    macEV
                    << "Last beacon was send at " << mpib.getMacBeaconTxTime() << " actual CAP is " << tmpf << endl;
                }
        ...
        else  // check RX CAP
        {
            if (rxBO != 15)  // beacon enabled
            {
                ...
                {
                    macEV
                    << "During receiving a beacon, now is: " << now << ", last beacon was received at " << bcnRxTime << "\n";
                    while (rxCAP < now)
                    {
                        rxCAP += rxBI;
                    }
...
void IEEE802154MacAttacker::csmaca_handle_RX_ON_confirm(phyState status)
{
    simtime_t now, wtime;
...
void IEEE802154MacAttacker::csmacaTrxBeacon(char trx)
{
    simtime_t wtime;
    if (!txAck)
    {
...
void IEEE802154MacAttacker::startBackoffTimer(simtime_t wtime)
{
    ASSERT(tmpCsmaca != NULL);
...
void IEEE802154MacAttacker::startRxOnDurationTimer(simtime_t wtime)
{
...
void IEEE802154MacAttacker::startdeferRxEnableTimer(simtime_t wtime)
{
...
void IEEE802154MacAttacker::startDeferCCATimer(simtime_t wtime)
{
...
void IEEE802154MacAttacker::startBcnRxTimer()
{
    simtime_t now, wtime;
    now = simTime();
...
void IEEE802154MacAttacker::startBcnTxTimer(bool txFirstBcn, simtime_t startTime)
{
    if (mpib.getMacBeaconOrder() == 15)
    {
        return;
...
void IEEE802154MacAttacker::startAckTimeoutTimer()
{
...
void IEEE802154MacAttacker::startTxAckBoundTimer(simtime_t wtime)
{
...
void IEEE802154MacAttacker::startIfsTimer(IFSType ifsType)
{
    ...
    macEV
    << "startIfsTimer for IFS Type: " << ((ifsType == SIFS) ? "SIFS" : "LIFS") << " with wtime: " << wtime << endl;
...
void IEEE802154MacAttacker::startTxSDTimer()
{
...
void IEEE802154MacAttacker::startRxSDTimer()
{
...
void IEEE802154MacAttacker::startScanTimer(simtime_t wtime)
{
    ASSERT(scanning);
...
unsigned char IEEE802154MacAttacker::calcFrameByteLength(cPacket* frame)
{
    ...
    macEV
    << "MAC FrameSize = MHR (" << (int) MHRLength << " Bytew) + Payload (" << (int) (byteLength - MHRLength - 2) << " Bytes) + MFR (2 Bytes) = " << (int) byteLength << " Bytes \n";

    return byteLength;
}

unsigned char IEEE802154MacAttacker::calcMacHeaderByteLength(unsigned char addrModeSum, bool secu)
{
    int ashSize = 0;
    if (secu)
    {
...
/******************************* <Timer Handler> ********************************/
//--------------------------------------------------------------------------------/
void IEEE802154MacAttacker::handleBackoffTimer()
{
...
void IEEE802154MacAttacker::handledeferRxEnableTimer()
{
...
void IEEE802154MacAttacker::handleRxOnDurationTimer()
{
    genSetTrxState(phy_TRX_OFF);
    return;
}

void IEEE802154MacAttacker::handleDeferCCATimer()
{
    taskP.taskStatus(TP_CCA_CSMACA) = true;
...
void IEEE802154MacAttacker::handleBcnRxTimer()
{
...
void IEEE802154MacAttacker::handleBcnTxTimer()
{
 ...
 */
void IEEE802154MacAttacker::handleTxCmdDataBoundTimer()
{
 ...
 */
void IEEE802154MacAttacker::handleIfsTimer()
{
...
void IEEE802154MacAttacker::handleSDTimer() // we must make sure that outgoing CAP and incoming CAP never overlap
{
    inTxSD_txSDTimer = false;
    inRxSD_rxSDTimer = false;
...
void IEEE802154MacAttacker::handleScanTimer()
{
    doScan();
}
 ...
 */
void IEEE802154MacAttacker::gtsScheduler()
{
//unsigned char t_SO;
...
// should turn on radio receiver aTurnaroundTime symbols before GTS starts, if it is a transmit GTS relative to device
    ...
    macEV
    << "[GTS]: Schedule to start GTS index: " << index_gtsTimer << ", slot:#" << (int) gtsList[index_gtsTimer].startSlot << " with " << w_time << " sec \n";
    startGtsTimer(w_time);
}

void IEEE802154MacAttacker::startGtsTimer(simtime_t w_time)
{
...
void IEEE802154MacAttacker::handleGtsTimer()
{
    simtime_t w_time;
    ...
    if (isCoordinator)  // for PAN coordinator
    {
        ...
        macEV
        << "allocated for MAC address = " << (int) gtsList[indexCurrGts].devShortAddr << ", startSlot = " << (int) gtsList[indexCurrGts].startSlot << ", length = " << (int) gtsList[indexCurrGts].length << ", isRecvGTS = " << gtsList[indexCurrGts].isRecvGTS << ", isTxPending = " << gtsList[indexCurrGts].isTxPending << endl;
 ...
 */
unsigned char IEEE802154MacAttacker::gts_request_cmd(unsigned short macShortAddr, unsigned char length, bool isReceive)
{
    Enter_Method_Silent
    ();
...
// check if the minCAP length can still be satisfied after this allocation
    ...
    {
        macEV
        << "[GTS]: violation of the min CAP length, no GTS allocated for the MAC address " << macShortAddr << endl;
        return 0;
    }
...
// add new GTS descriptor
    ...
    macEV
    << "[GTS]: add a new GTS descriptor with index = " << (int) gtsCount << " for MAC address = " << macShortAddr << ", startSlot = " << (int) startSlot << ", length = " << (int) length << ", isRecvGTS = " << isReceive << endl;
    gtsCount++;
...
void IEEE802154MacAttacker::startFinalCapTimer(simtime_t w_time)
{
...
void IEEE802154MacAttacker::handleFinalCapTimer()
{
 ...
 */
bool IEEE802154MacAttacker::gtsCanProceed()
{
    simtime_t t_left;
...
//+++++++ https://www7.informatik.uni-erlangen.de/~fengchen/omnet/802154/ +++++++/
//-------------------------------------------------------------------------------/
void IEEE802154MacAttacker::taskSuccess(char type, bool csmacaRes)
{
    if (type == 'b')  //beacon
    {
    ...
    else if (type == 'g')  // GTS
    {
        ...
        ASSERT(txGTS);
        macEV
        << "[GTS]: taskSuccess for sending : " << txGTS->getName() << ":#" << (unsigned int) txGTS->getSqnr() << endl;
...
void IEEE802154MacAttacker::taskFailed(char type, MACenum status, bool csmacaRes)
{
    if ((type == 'b') // Beacon
    || (type == 'a')  // ACK
    || (type == 'c')) // Command
...
void IEEE802154MacAttacker::checkTaskOverflow(Ieee802154MacTaskType task)
{
...
void IEEE802154MacAttacker::FSM_MCPS_DATA_request(phyState pStatus, MACenum mStatus)
{
    Ieee802154MacTaskType task = TP_MCPS_DATA_REQUEST;
    ...
    if (txOption == DIRECT_TRANS)
    {
        switch (taskP.taskStep(task))
        {
            ...
            case 3: {
                ...
                {
                    macEV
                    << "ACK for " << txData->getName() << ":#" << (unsigned int) txData->getSqnr() << " received successfully \n";
                    numRxAckPkt++;
                    taskP.taskStatus(task) = false;  // task ends successfully
                ...
                {
                    macEV
                    << "ACK timeout for " << txData->getName() << ":#" << (unsigned short) txData->getSqnr() << endl;
                    numDataRetry++;
                    if (numDataRetry <= mpib.getMacMaxFrameRetries())
                    {
                        macEV
                        << "Starting " << numDataRetry << ". retry for " << txData->getName() << ":#" << (unsigned short) txData->getSqnr() << endl;
                    ...
                    {
                        macEV
                        << "The max #num of retries (MaxFrameRetries=" << mpib.getMacMaxFrameRetries() << ") reached \n";
        ...
        switch (taskP.taskStep(task))
        {
            ...
            case 3: {
                if (pStatus == phy_SUCCESS)  // ACK received
                {
                    waitGTSAck = false;
                    macEV
                    << "[GTS]: ACK for " << txGTS->getName() << ":#" << (unsigned int) txGTS->getSqnr() << " received successfully, turn off radio now \n";
                    numRxAckPkt++;
                    genSetTrxState(phy_FORCE_TRX_OFF);
                    macEV << "[GTS]: need to delay IFS before next GTS transmission can proceed \n";
                    taskP.taskStep(task)++; // advance to next task step
                ...
                {
                    macEV
                    << "[GTS]: ACK timeout for " << txGTS->getName() << ":#" << (unsigned int) txGTS->getSqnr() << endl;
                    numGTSRetry++;
                    ...
                    {
                        macEV
                        << "[GTS]: the #max num of retries (MacMaxFrameRetries=" << mpib.getMacMaxFrameRetries() << ") reached, task failed \n";
                        taskP.taskStatus(task) = false;  //task fails
                        macEV << "[MAC-TASK-FAIL]: reset TP_MCPS_DATA_REQUEST \n";
                        waitGTSAck = false;
                        // to avoid several consecutive genSetTRXState are called at the same time, which may lead to error operation,
...
void IEEE802154MacAttacker::dispatch(phyState pStatus, const char *frFunc, phyState req_state, MACenum mStatus)
{
    /*****************************************/
    /************<csmacaCallBack()>**********/
    /***************************************/
...
void IEEE802154MacAttacker::resetTRX()
{
    phyState t_state;
...
std::string IEEE802154MacAttacker::AEADCypher32(std::string adata, std::string pdata)
{
...
std::string IEEE802154MacAttacker::AEADCypher64(std::string adata, std::string pdata)
{
...
std::string IEEE802154MacAttacker::AEADCypher128(std::string adata, std::string pdata)
{
...
std::string IEEE802154MacAttacker::CTRCypher(std::string pdata)
{
...
std::string IEEE802154MacAttacker::AEADDecypher32(std::string cipher, std::string radata)
{
    ...
    try
    {
        ...
        //  during decryption\verification _if_
        //  verification fails.
        AuthenticatedDecryptionFilter df(d, NULL, AuthenticatedDecryptionFilter::MAC_AT_BEGIN | AuthenticatedDecryptionFilter::THROW_EXCEPTION); // AuthenticatedDecryptionFilter

        // The order of the following calls are important
        //  when using the MAC_AT_BEGIN flag
        df.ChannelPut(DEFAULT_CHANNEL, reinterpret_cast<const unsigned char*>(tag.data()), tag.size());
        df.ChannelPut(AAD_CHANNEL, reinterpret_cast<const unsigned char*>(radata.data()), radata.size());
        df.ChannelPut(DEFAULT_CHANNEL, reinterpret_cast<const unsigned char*>(enc.data()), enc.size());
...
std::string IEEE802154MacAttacker::AEADDecypher64(std::string cipher, std::string radata)
{
...
std::string IEEE802154MacAttacker::AEADDecypher128(std::string cipher, std::string radata)
{
...
std::string IEEE802154MacAttacker::CTRDecypher(std::string cypher)
{
...
std::string IEEE802154MacAttacker::CBCMACAuth32(std::string plain)
{
...
std::string IEEE802154MacAttacker::CBCMACAuth64(std::string plain)
{
...
std::string IEEE802154MacAttacker::CBCMACAuth128(std::string plain)
{
...
void IEEE802154MacAttacker::CBCMACVerify32(std::string plain, std::string mac)
{
...
void IEEE802154MacAttacker::CBCMACVerify64(std::string plain, std::string mac)
{
...
void IEEE802154MacAttacker::CBCMACVerify128(std::string plain, std::string mac)
{
...
void IEEE802154MacAttacker::setAPDATA(std::string *adata, std::string *pdata, mpdu *frame, bool encryption)
{
    ...
    else if (strcmp(frame->getName(), "ASSOCIATE-request-command") == 0)
    {
        ...
        *adata += '#';
        ...
        if (encryption)
        {
            ...
            // content= * * device type * power source * rowi * * * security capability * allocate address * allpancoor * hostname
            ...
            *pdata += tmp.powSrc;
            *pdata += '*';
            *pdata += tmp.recvOnWhenIdle;
            *pdata += '*';
            *pdata += '*';
            *pdata += '*';
            *pdata += tmp.secuCapable;
            *pdata += '*';
...
void IEEE802154MacAttacker::setADATA(std::string *adata, mpdu *frame)
{
...
void IEEE802154MacAttacker::setADATAck(std::string *adata, AckFrame* ack)
{
...
std::string IEEE802154MacAttacker::secPacket(mpdu *frame)
{
...
std::string IEEE802154MacAttacker::secRecPacket(mpdu *frame)
{
...
std::string IEEE802154MacAttacker::secAck(AckFrame* ack, int secuLevel)
{
...
void IEEE802154MacAttacker::secRecAck(AckFrame* ack, int secuLevel)
{
    std::string radata;
    std::string decipherT;
    std::stringstream temp;
...
void IEEE802154MacAttacker::printHex(std::string text)
{
...
vector<string> IEEE802154MacAttacker::parserSecMessage(std::string str, char ch)
{
...
int IEEE802154MacAttacker::calcByteMicLenght(bool securityEnable, int secuLevel)
{
...
int IEEE802154MacAttacker::calcBytePayload(int type, bool securityEnable, int secuLevel)
{
...
int IEEE802154MacAttacker::registerBattery()
{
...
void IEEE802154MacAttacker::drawBattery(int activity)
{
    ...
    if (bat)
    {
        ...
        switch (activity)
        {
            case IEEE802154MacAttacker::BEACON_TEMP:
                currentDraw =
                {   DrawAmount::CURRENT, PacketCurrent};
                bat->drawMAC(idBattery, currentDraw, IEEE802154MacAttacker::BEACON_TEMP, beaconTemp);
                break;
                case IEEE802154MacAttacker::ACK_TEMP:
                currentDraw =
                {   DrawAmount::CURRENT, AckCurrent};
                bat->drawMAC(idBattery, currentDraw, IEEE802154MacAttacker::ACK_TEMP, ackTemp);
                break;
                case IEEE802154MacAttacker::DATA_TEMP:
                currentDraw =
                {   DrawAmount::CURRENT, PacketCurrent};
                bat->drawMAC(idBattery, currentDraw, IEEE802154MacAttacker::DATA_TEMP, dataTemp);
                break;
                case IEEE802154MacAttacker::ASSORESP_TEMP:
                currentDraw =
                {   DrawAmount::CURRENT, PacketCurrent};
                bat->drawMAC(idBattery, currentDraw, IEEE802154MacAttacker::ASSORESP_TEMP, assorespTemp);
                break;
                case IEEE802154MacAttacker::ASSOREQ_TEMP:
                currentDraw =
                {   DrawAmount::CURRENT, PacketCurrent};
                bat->drawMAC(idBattery, currentDraw, IEEE802154MacAttacker::ASSOREQ_TEMP, assoreqTemp);
                break;
...
bool IEEE802154MacAttacker::checkSecFrameCounter(std::string mac, unsigned int frameCounter)
{
    if (replayProtection)
    {
...
bool IEEE802154MacAttacker::increaseFrameCounter(unsigned int * ashFrameCount)
{
...
IEEE802154MacAttacker::IEEE802154MacAttacker()
{
    txPkt = NULL;
    txBeacon = NULL;
    txBcnCmdUpper = NULL;
