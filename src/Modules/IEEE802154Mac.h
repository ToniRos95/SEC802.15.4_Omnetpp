...
protected:
    ...
    unsigned short genFCF(frameType ft, bool secu, bool fp, bool arequ,
            bool pid, AddrMode dam, unsigned short fv, AddrMode sam);
    ...
    unsigned short calcFCS(mpdu* pdu, cMessage *msg, bool calcFlag,
            int headerSize);
    ...
    bool gtsCanProceed();
    void gtsScheduler();
    unsigned char gts_request_cmd(unsigned short macShortAddr,
            unsigned char length, bool isReceive);
    simtime_t getFinalCAP(char trxType);
    MACAddressExt associate_request_cmd(MACAddressExt extAddr,
     ...
     */
    void dispatch(phyState pStatus, const char *frFunc, phyState req_state =
            phy_SUCCESS, MACenum mStatus = mac_SUCCESS);
    ...
    void FSM_MCPS_DATA_request(phyState pStatus = phy_SUCCESS, MACenum mStatus =
            mac_SUCCESS);
    void resetTRX();
