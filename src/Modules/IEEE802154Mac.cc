...
void IEEE802154Mac::initialize(int stage) {
    ...
    if (stage == 0) {
        ...
        macDebug =
                (hasPar("macDebug") ? (par("macDebug").boolValue()) : (false));
        ...
        unsigned int hostIndex =
                this->getParentModule()->getParentModule()->getParentModule()->getIndex();
        ...
        macEV
        << "Initialization for MacDSN = " << mpib.getMacDSN()
                << " and MacBSN = " << mpib.getMacBSN() << endl;
        ...
        mpib.setMacAckWaitDuration(
                aUnitBackoffPeriod + aTurnaroundTime
                        + (getModuleByPath("^.^.PHY")->par("SHRDuration").longValue())
                        + (6
                                - (getModuleByPath("^.^.PHY")->par(
                                        "symbolsPerOctet").longValue())));

        // inizialize PHY-related variables from PHY.ned -> FIXME -> PLME-GET msg should be used
        phy_channel =
                getModuleByPath("^.^.PHY")->par("currentChannel").longValue();
        ...
        if (myMacAddr.isUnspecified()) {
            if (!strcmp(addressString, "auto")) {
                ...
                myMacAddr = MACAddressExt::generateMacAddressWithNodeIndex(
                        getModuleByPath("^.^.^.")->getIndex());
                macEV
                << "myMacAddr is unspecified -> assigning generated address = "
                        << myMacAddr << endl;
                ...
                macEV
                        << "myMacAddr is unspecified -> assigning user set address (macAddr in omnetpp.ini) = "
                        << myMacAddr << endl;
            }
        }
        ...
        unsigned short llcTXoption =
                getModuleByPath("^.^.^.Network.stdLLC")->par("TXoption");
        ...
        {
            error(
                    "FIXME check if txOption => 6 and 7 are valid cases at all according to the standard \n");
        }
        default: {
        ...
        if (true == startNow) {
            ...
            if (true == isCoordinator) {
                if (false == isFFD) {
                    error(
                            "[IEEE802154MAC]: you want to start up a PAN Coordinator who is not an FFD!!!");
                }
                ...
                if (mpib.getMacBeaconOrder() < 15) {
                    ...
                    txSfSlotDuration = aBaseSlotDuration
                            * (1 << mpib.getMacSuperframeOrder());
                    ...
                    error(
                            "[IEEE802154MAC]: undefined Beacon Order found during startup");
                }
            }  // if == isCoordinator
            else {
...
void IEEE802154Mac::receiveChangeNotification(int category,
    ...
    case NF_RADIO_CHANNEL_CHANGED: {
        phy_channel =
                check_and_cast<const RadioState *>(details)->getChannelNumber();
        phy_bitrate = getRate('b');
        phy_symbolrate = getRate('s');
...
void IEEE802154Mac::setDefMpib() {
    ...
    mpib.setMacAckWaitDuration(
            aUnitBackoffPeriod + aTurnaroundTime
                    + (int) (getModuleByPath("^.^.PHY")->par("SHRDuration"))
                    + (6
                            - (int) (getModuleByPath("^.^.PHY")->par(
                                    "symbolsPerOctet"))));
...
void IEEE802154Mac::handleMessage(cMessage *msg) {
    if (ev.isGUI()) {
        char bufTT[50];
        sprintf(bufTT, "isCoordinator: %s | isAssociated: %s",
...
void IEEE802154Mac::handleUpperMsg(cMessage *msg) {
    ...
    if (taskP.taskStatus(TP_MCPS_DATA_REQUEST) || scanning) {
        macEV
        << "A " << msg->getName() << " (#" << numUpperPkt
                << ") received from upper layer, but dropped due to concurrent msg processing \n";
        numUpperPktLost++;
        delete msg;
    ...
    if (msg->arrivedOn("MCPS_SAP")) {
        ...
        macEV
        << "MCPS-DATA.request arrived on MCPS_SAP for "
                << dataReq->getDstAddr().str() << endl;

        // Check if the data request packet has a valid size
        if (dataReq->getByteLength() > aMaxMACFrameSize) {
            macEV
            << dataReq->getName()
                    << " received from upper layer, but dropped due to oversize \n";
        ...
        case 0:  // direct CAP noAck
        {
            ...
            data->setFcf(
                    genFCF(Data, false, false, false, false, addrLong, 01,
                            addrLong));
        ...
        case 1:  // direct CAP ACK
        {
            taskP.mcps_data_request_TxOption = DIRECT_TRANS;
            data->setFcf(
                    genFCF(Data, false, false, true, false, addrLong, 01,
                            addrLong));
            data->setByteLength(calcFrameByteLength(data));
            data->setIsGTS(false);
            waitDataAck = true;
        ...
        case MLMEASSOCIATE: {
            if (isCoordinator) {
                ...
                AssociationRequest* frame =
                        check_and_cast<AssociationRequest *>(msg);
                AssoCmdreq* AssoCommand = new AssoCmdreq(
                        "ASSOCIATE-request-command");
                MACAddressExt dest;
                switch (frame->getCoordAddrMode()) {
                case addrShort: {
                    mpib.setMacCoordShortAddress(
                            frame->getCoordAddress().getShortAddr());
        ...
        case MLMEASSOCIATERESP: {
            if (isCoordinator) {
                ...
                assoCmdResp->setShortAddress(
                        assoResp->getAddr().getShortAddr());
                ...
                default: {
                    error("[IEEE802154MAC]: undefined txOption: %d!",
                            dataTransMode);
                    break;
                }
                }  // switch (dataTransMode)
                ...
                macEV
                        << "Device received 'Association Response' from Higher Layer - How could that happen?\n";
            }
        ...
        case MLMEDISASSOCIATE: {
            ...
            if (disAss->getDevicePANId() == mpib.getMacPANId()) {
                if (isCoordinator) {
                    ...
                    if (((disAss->getDeviceAddrMode() == addrLong)
                            && (disAss->getDeviceAddress()
                                    == mpib.getMacCoordExtendedAddress()))
                            || ((disAss->getDeviceAddrMode() == addrShort)
                                    && disAss->getDeviceAddress().getShortAddr()
                                            == mpib.getMacCoordShortAddress())) {
        ...
        case MLMEGTS: {
            if (isCoordinator) {
                ...
                if (associated) {
                    ...
                    gtsRequCmd->setFcf(
                            genFCF(Command, mpib.getMacSecurityEnabled(), false,
                                    true, false, addrShort, 0, addrShort));
                } else {
                    genGTSConf(GTSrequ->getGTSCharacteristics(),
        ...
        case MLMERXENABLE: {
            ...
            if (dataTransMode == INDIRECT_TRANS) {
                ...
                if (rxReq->getRxOnTime() + rxReq->getRxOnDuration()
                        > aBaseSuperframeDuration
                                * pow(2, mpib.getMacBeaconOrder())) {
                    RxEnableConfirm* conf = new RxEnableConfirm(
                            "MLME-RX-ENABLE.confirm");
                    conf->setStatus(rx_ON_TIME_TOO_LONG);
                    send(conf, "outMLME");
                    delete (msg);    // fix for undisposed object message
                    return;
                } else if ((rxReq->getRxOnTime() - aTurnaroundTime)
                        < (bcnRxTime - simTime())) // TODO adapt to symbols to simtime_t conversion
                        {
                    ...
                    startdeferRxEnableTimer(
                            rxReq->getRxOnTime() - aTurnaroundTime);
                    deferRxEnable = rxReq;
                    delete (msg);    // fix for undisposed object message
                    return;
                } else {
            ...
            if (scanning) {
                ...
                if (!isFFD) {
                    if (scanType == scan_ORPHAN) {
                        ...
                        ScanConfirm* scanConf = new ScanConfirm(
                                "MLME-SCAN.confirm");
        ...
        case MLMEPOLL: {
            ...
            dataReq->setFcf(
                    genFCF(Command, false, false, false, false, addrLong, 0,
                            addrLong));
...
void IEEE802154Mac::handleLowerPLMEMsg(cMessage* msg) {
    if (mappedMsgTypes[msg->getName()] == CCA) {
        ...
        return;
    } else if ((mappedMsgTypes[msg->getName()] == ED)
            && (dynamic_cast<edConf*>(msg))) {
        edConf* edConfirmation = check_and_cast<edConf*>(msg);
        if (scanning) {
            scanEnergyDetectList[scanResultListSize] =
                    edConfirmation->getEnergyLevel();
            scanResultListSize++;
        } else {
            send(edConfirmation, "outMLME");
        }
        return;
    } else if ((mappedMsgTypes[msg->getName()] == GET)
            && (dynamic_cast<GetConfirm*>(msg))) {
        handle_PLME_GET_confirm(msg);
        return;
    } else if ((mappedMsgTypes[msg->getName()] == SET)
            && (dynamic_cast<SetConfirm*>(msg))) {
        handle_PLME_SET_confirm(msg);
        return;
    } else if ((!msg->isPacket())
            && (mappedMsgTypes[msg->getName()] == SETTRXSTATE)) {
        handle_PLME_SET_TRX_STATE_confirm((phyState) msg->getKind());
        delete (msg);   // XXX fix for undisposed objects
        return;
    } else {
        error(
                "[IEEE802154MAC]: undefined message in handleLowerPLMEMsg -> kind: %d | name: %s \n",
...
void IEEE802154Mac::handleLowerPDMsg(cMessage* msg) {
    if (dynamic_cast<CmdFrame*>(msg)) {
        ...
        if (bcnReq->getCmdType() == Ieee802154_BEACON_REQUEST) // it is a beacon request command
            ...
            if (isCoordinator) {
                macEV
                        << "Coordinator is answering with a Beacon frame for this Device \n";
                ...
                txSfSpec.BI = aBaseSuperframeDuration
                        * (1 << mpib.getMacBeaconOrder());
                txSfSpec.SO = mpib.getMacSuperframeOrder();
                txSfSpec.SD = aBaseSuperframeDuration
                        * (1 << mpib.getMacSuperframeOrder());
                ...
                if (txBcnCmd != NULL) {   // will be released in PD_DATA_confirm
                    macEV
                            << "Coordinator is already trying to answer on BEACON.request - inserting this BEACON into txBuffer \n";
    ...
    if (!frame) {
        macEV
        << "Message from PHY (" << msg->getClassName() << ")" << msg->getName()
                << " is not any known subclass, drop it \n";
        delete (frame);
        return;
    }
    ...
    macEV
            << (frmType == Beacon ?
                    "Beacon" :
                    (frmType == Data ?
                            "Data" : (frmType == Ack ? "Ack" : "Command")))
            << " frame received from PHY, start filtering now\n";
    ...
    if (frmType == Data && frame->getIsGTS()) {
        if (isCoordinator) {
            ...
            if (indexCurrGts == 99 || gtsList[indexCurrGts].isRecvGTS
                    || gtsList[indexCurrGts].devShortAddr
                            != frame->getSrc().getShortAddr()) {
                error(
                        "[GTS]: timing error, PAN coordinator is not supposed to receive this DATA packet at this time!");
            }
        } else {
            if (index_gtsTimer != 99 || !isRecvGTS
                    || frame->getSrc().getShortAddr()
                            != mpib.getMacCoordShortAddress()) {
                error(
                        "[GTS]: timing error, the device is not supposed to receive this DATA packet at this time!");
            }
        }
    }
    ...
    if ((frmType == Data) || (frmType == Command)) {
        (frmType == Data) ?
        macEV << "Checking if received data frame requires ACK \n" :
        ...
        if ((frmCtrl & arequMask) >> arequShift) // check if acknowledgment required
            ...
            if (!noAck) {
                ...
                if (backoffStatus == 99) {
                    macEV
                            << "CSMA-CA is pending, stop it, it will resume after sending ACK \n";
                    backoffStatus = 0;
                    csmacaCancel();
                }
                macEV
                        << "Prepare to send the ACK, ask PHY layer to turn on the transmitter first \n";
    ...
    if (frmType == Command) {
        if ((rxCmd) || (txBcnCmd)) {
            macEV
                    << "Received CMD frame is dropped, because MAC is currently processing a MAC CMD \n";
            delete (frame);
            return;
        }
    }
    ...
    if (frmType == Data) {
        if (rxData) {
            macEV
                    << "Received DATA frame is dropped, because MAC is currently processing the last received DATA frame \n";
            delete (frame);
            return;
        }
    }
...
void IEEE802154Mac::handleBeacon(mpdu *frame) {
    ...
    if (mpib.getMacSecurityEnabled()) {
        ...
        std::string prova = secRecPacket(bcnFrame, bcnFrame->getName(), bcnFrame->getPayload());
    ...
    if (scanning) {
        ...
        scanResultListSize++;
        macEV
                << "Beacon frame was processed while scanning the channel - scanResultList was updated \n";
    } else {
        unsigned short frmCtrl = frame->getFcf();
        ...
        macEV
        << "The first bit of this beacon was received by PHY layer at "
                << bcnRxTime << endl;

        simtime_t tmpf = duration * phy_symbolrate;

        // calculate <rxBcnDuration>
        (bcnFrame->getByteLength() <= aMaxSIFSFrameSize) ?
                tmpf += aMinSIFSPeriod : tmpf += aMinLIFSPeriod;
        ...
        if (gtsStartSlot != 0) {
            ...
            macEV
            << "Schedule for my GTS with start slot #" << (int) gtsStartSlot
                    << "\n";
            ...
            if (waitGTSConf) {
                ...
                while (bcnFrame->getGtsList(i).devShortAddr
                        != myMacAddr.getShortAddr() && i < 7) {
                    i++;
                }
                if (i < 7) {
            ...
            if (gtsStartSlot != rxSfSpec.finalCap + 1) {
                ...
                w_time = tmpf - now;
                macEV
                        << "My GTS is not the first one in the CFP, schedule a timer to turn off radio at the end of CAP \n";
...
void IEEE802154Mac::handleAck(cMessage* frame) {
    ...
    if ((ack4me == true) && (waitDataAck == true || waitGTSAck == true)) {
        ...
        macEV
        << "ACK #" << (unsigned int) ack->getSqnr()
                << " does not match a sequence number of a packet to transmit --> drop the ACK \n";
    } else if (waitDataAck == false && waitGTSAck == false) {
        macEV
        << "ACK #" << (unsigned int) ack->getSqnr()
                << " received while node is not waiting for an ACK --> drop the ACK \n";
    }
...
// (refer to Figure 68 of the 802.15.4-2006 Spec. for details of SIFS/LIFS)
    ...
    if (!((frmCtrl & arequMask) >> arequShift)) {
        (frame->getByteLength() <= aMaxSIFSFrameSize) ?
                startIfsTimer(SIFS) : startIfsTimer(LIFS);
    }
    return;
}
...
// all commands in this environment are encapsulated in MPDU
    ...
    case Ieee802154_ASSOCIATION_REQUEST: {
        ...
        if (isCoordinator) {
            ...
            assoInd->setCapabilityInformation(
                    tmpAssoReq->getCapabilityInformation());
            ...
            macEV
                    << "Device is dropping Association Request frame since we are NOT a coordinator \n";
            delete (rxCmd); // fix for undisposed object (mpdu)
            rxCmd = NULL;  // XXX delete command message buffer after processing
            delete (cmdFrame);
            return;
    ...
    case Ieee802154_ASSOCIATION_RESPONSE: {
        ...
        if (isCoordinator) {
            ...
            AssociationConfirm* assoConf = new AssociationConfirm(
    ...
    case Ieee802154_DISASSOCIATION_NOTIFICATION: {
        ...
        DisAssociation* assoInd = new DisAssociation(
    ...
    case Ieee802154_GTS_REQUEST: {
        if (isCoordinator) {
            ...
            gts_request_cmd(gtsRequ->getGTSCharacteristics().devShortAddr,
                    gtsRequ->getGTSCharacteristics().length, true);
            // gen indication
    ...
    default: {
        macEV
        << "Got unknown Command type in a Command Frame - dropping frame \n";
        rxCmd = NULL;   // XXX delete command message buffer after processing
        delete (cmdFrame);
        return;
    }  // default
...
void IEEE802154Mac::handle_PLME_SET_TRX_STATE_confirm(phyState status) {
    macEV
    << "PLME_SET_TRX_STATE_confirm with phyState " << phyStateToString(status)
            << " received from PHY, the requested state is "
            << phyStateToString(trx_state_req) << endl;
    simtime_t delay;
    ...
    } else if (txAck) // ACK needs to be transferred, check for possible backoffs
    {
        ...
        } else if ((rxData != NULL)) // beacon enabled PAN - processing DATA frame
        {
            ...
            delay = csmacaLocateBoundary(
                    (rxData->getSrc().getShortAddr()
                            == mpib.getMacCoordShortAddress()), 0.0);
            ASSERT(delay < bPeriod);
        } else if ((rxCmd != NULL)) // beacon enabled PAN - processing CMD frame
        {
            // here we use the hidden destination address that we already set in ACK on purpose
            delay = csmacaLocateBoundary(
                    (rxCmd->getSrc().getShortAddr()
                            == mpib.getMacCoordShortAddress()), 0.0);
            ASSERT(delay < bPeriod);
        }
...
void IEEE802154Mac::handle_PLME_CCA_confirm(phyState status) {
    macEV
    << "PLME-CCA.confirm with phyState " << phyStateToString(status)
            << " received from PHY \n";
...
void IEEE802154Mac::handle_PLME_GET_confirm(cMessage* msg) {
    ...
    macEV
    << "PLME-GET.confirm for attribute '"
            << PhyPIB_AttributesToString(
                    PhyPIB_Attributes(getConf->getPIBattr()))
            << "' received from PHY with status: "
            << PhyPIB_StatusToString(PhyPIB_Status(getConf->getStatus()))
    ...
    default: {
        error(
                "[802154MAC]: undefined PIB attribute value = %d in PLME-GET.confirm message received \n",
                getConf->getPIBattr());
        break;
    }
    }
...
void IEEE802154Mac::handle_PLME_SET_confirm(cMessage* msg) {
    ...
    macEV
    << "PLME-SET.confirm for attribute '"
            << PhyPIB_AttributesToString(
                    PhyPIB_Attributes(setConf->getPIBattr()))
            << "' received from PHY with status: "
            << PhyPIB_StatusToString(PhyPIB_Status(setConf->getStatus()))
            << endl;
    ...
    case channelSupported: {
        macEV
                << "additional handling for PLME-SET.confirm 'channelSupported' necessary? \n";
        break;
    }
    case transmitPower: {
        macEV
                << "additional handling for PLME-SET.confirm 'transmitPower' necessary? \n";
        break;
    }
    case CCA_Mode: {
        macEV
        << "additional handling for PLME-SET.confirm 'CCA_Mode' necessary? \n";
        break;
    }
    case currentPage: {
        macEV
                << "additional handling for PLME-SET.confirm 'currentPage' necessary? \n";
        break;
    }
    case maxFrameDuration: {
        macEV
                << "additional handling for PLME-SET.confirm 'maxFrameDuration' necessary? \n";
        break;
    }
    case SHRDuration: {
        macEV
                << "additional handling for PLME-SET.confirm 'SHRDuration' necessary? \n";
        break;
    }
    case symbolsPerSecond: {
        macEV
                << "additional handling for PLME-SET.confirm 'symbolsPerSecond' necessary? \n";
        break;
    }
    default: {
...
// perform further filtering only if the PAN is currently not in promiscuous mode
    if (!mpib.getMacPromiscuousMode()) {
        ...
        AddrMode addressMode = (AddrMode) (((pdu->getFcf()) & damMask)
                >> damShift);

        // check packet type
        if ((frmType != Beacon) && (frmType != Data) && (frmType != Ack)
                && (frmType != Command)) {
        ...
        if ((frmType == Ack) && (pdu->getSqnr() != mpib.getMacDSN())) {
            macEV
                    << "Further Filtering for frmType == Ack & Sqnr != MacDSN --> ACK packet filtered \n";
        ...
        if (frmType == Beacon) {
            ...
            && (pdu->getSrcPANid() != mpib.getMacPANId())) // PAN ID did not match
                    {
                macEV
                        << "Further Filtering for frmType == Beacon & PAN ID did not match --> BEACON packet filtered \n";
                return true;
            }
        } else {
            ...
            if ((addressMode == addrShort) || (addressMode == addrLong)) {
                ...
                && (pdu->getDestPANid() != mpib.getMacPANId())
                        && (mpib.getMacAssociatedPANCoord())) {
                    macEV
                            << "Further Filtering for frmType != Beacon & has PAN ID --> packet filtered \n";
                    return true;
                }
            }
            ...
            {
                if (!(pdu->getDest().isBroadcast())
                        && (pdu->getDest().getShortAddr()
                                != mpib.getMacShortAddress())) {
                    macEV
                            << "Further Filtering for frmType != Beacon & has Short Addr -> not the designated receiver --> packet filtered \n";
                    return true;
                }
            } else if ((addressMode == addrLong))  // extended address
            {
                if (!(pdu->getDest().equals(myMacAddr))) {
                    macEV
                            << "Further Filtering for frmType != Beacon & has Long Addr -> not the designated receiver --> packet filtered \n";
                    return true;
                }
            }
        }
        ...
        if ((frmType == Data) || (frmType == Command)) {
            if (addressMode == noAddr) // destination address fields not included
                ...
                if (!isCoordinator) {
                    ...
                    macEV
                            << "Further Filtering for frmType == Data || Command & addressMode == none & no Coordinator --> filtered here \n";
                    return true;
                }
            }
            macEV
                    << "Further Filtering for frmType == Data || Command --> okay, NOT filtered \n";
        }
    }
    return false;
}
...
unsigned short IEEE802154Mac::genFCF(frameType ft, bool secu, bool fp,
        bool arequ, bool pid, AddrMode dam, unsigned short fv, AddrMode sam) {
...
void IEEE802154Mac::genSetTrxState(phyState state) {
    ...
    trx_state_req = state;
    macEV
    << "Sending SET-TRX-STATE.request to PLME with Transceiver_State="
            << phyStateToString(state) << endl;
    send(setTrx, "outPLME");
    return;
}
...
// Frame Control Field: Page 147 of 2006 revision says all fields except frame pending shall be zero
    ack->setFcf(
            genFCF(Ack, mpib.getMacSecurityEnabled(), fp, false, false, noAddr,
                    00, noAddr));
    ack->setFcs(0);
...
void IEEE802154Mac::genMLMEDisasConf(MACenum status) {
    DisAssociation* disAssConf = new DisAssociation(
...
void IEEE802154Mac::genAssoResp(MlmeAssociationStatus status,
        AssoCmdreq* tmpAssoReq) {
    ...
    assoResp->setFcf(
            genFCF(Command, mpib.getMacSecurityEnabled(), false, true, false,
                    addrLong, 0, addrShort));
    ...
    macEV
    << "Generating MLME-ASSOCIATE.response for: " << devAddr
            << " with short address: " << devAddr.getShortAddr()
            << " and #seqNr: " << (int) assoResp->getSqnr() << endl;
...
// because prompt GTS transfer leads to collisions with the immediate transport of ACK frames
    if ((txData != NULL) || (txGTS != NULL)) {
        macEV
                << "Processing other data right now -> associate response added to txBuffer, will be transmitted later \n";
...
void IEEE802154Mac::genDisAssoCmd(DisAssociation* disAss, bool direct) {
    ...
    if (disAss->getDeviceAddrMode() == addrShort) {
        disCmd->setFcf(
                genFCF(Command, mpib.getMacSecurityEnabled(), false, true,
                        false, addrShort, 0, addrShort));
        ...
        if ((txData != NULL) || (txGTS != NULL)) {
            macEV
                    << "Processing other data right now -> disasscoiate request will be transmitted later -> inserted into txBuffer \n";
            txBuffer.insert(holdMe);
        } else {
...
unsigned short IEEE802154Mac::calcFCS(mpdu* pdu, cMessage *payload,
        bool calcFlag, int headerSize) {
...
// we ignore Notification and send Indication instead
    ...
    holdMe->setSrcPANid(mpib.getMacPANId());
    holdMe->setFcf(
            genFCF(Command, false, false, false, false, addrLong, 0, addrLong));
...
void IEEE802154Mac::sendDown(mpdu* frame) {
    ...
    macEV
    << "Sending frame " << frame->getName() << " (" << frame->getByteLength()
            << " Bytes | Seq #" << (int) frame->getSqnr()
            << ") to PHY layer \n";
    macEV
    << "The estimated transmission time is " << calcDuration(frame)
            << " sec \n";
    send(frame, "outPD");  // send a duplication
}

void IEEE802154Mac::doScan() {
    switch (scanType) {  // Step 1 set TRX state 2 set channel 3
    case scan_ED: {
        if (scanStep == 0) {
            ...
            if (scanCount > 26) {
                macEV
                << "ED channel scan done - sending results to higher layer \n";
            ...
            send(edMsg, "outPLME");
            simtime_t wtime = (aBaseSuperframeDuration
                    * (pow(2, scanDuration) + 1) / phy_symbolrate);
    ...
    case scan_ACTIVE: {
        ...
        if (scanStep == 0) {
            ...
            if (scanCount > 26) {
                macEV
                << "Active channel scan done sending results to upper layer \n";
                genScanConf(scan_SUCCESS);
                scanStep = 0;
                scanning = false;
                reqMsgBuffer();
    ...
    case scan_PASSIVE: {
        ...
        if (scanStep == 0) {
            ...
            if (scanCount > 26) {
                macEV
                        << "Passive channel scan done - sending results to higher layer \n";
                genScanConf(scan_SUCCESS);
                scanStep = 0;
                scanning = false;
                reqMsgBuffer();
    ...
    case scan_ORPHAN: {
        ...
        if (scanStep == 0) {
            ...
            if (scanCount > 26) {
                macEV
                        << "Passive channel scan done sending results to upper layer \n";
                genScanConf(scan_SUCCESS);
                scanStep = 0;
                scanning = false;
                reqMsgBuffer();
...
void IEEE802154Mac::csmacaStart(bool firsttime, mpdu* frame, bool ackReq) {
    ...
    if (firsttime) {
        macEV
        << "[CSMA]: Starting CSMA-CA for " << frame->getName() << ":#"
                << (unsigned short) frame->getSqnr() << endl;
    ...
    macEV
    << "[CSMA]: Choosing random number of backoff periods: "
            << (wtime / bPeriod) << endl;
    macEV
    << "[CSMA]: Backoff time before adjusting: " << wtime * 1000 << " ms \n";
    ...
    if (beaconEnabled) {
        if (firsttime) {
            ...
            macEV
            << "[CSMA]: Backoff time after locating boundary: " << wtime * 1000
                    << " ms \n";
        }
...
// e.g. in handle_PLME_SET_TRX_STATE_confirm while preparing to send an ACK
    ...
    if (newtime > 0.0) {
        macEV
        << "[CSMA]: Deviation to backoff boundary: " << (bPeriod - newtime)
                << " sec \n";
...
// check 3: if the entire transmission can be finished before the end of current CAP
    ...
    if (!toParent(tmpCsmaca))  // as a coordinator
        ...
        if (mpib.getMacBeaconOrder() != 15)  // beacon enabled as a coordinator
                {
            t_fCAP = mpib.getMacBeaconTxTime()
            ...
            if (now >= t_fCAP)  // Check 1
                ...
                macEV
                        << "[CSMA]: Cannot proceed because it's now NOT in CAP (outgoing), resume at the beginning of next CAP \n";
                return false;
            } else  // Check 2
            {
                ...
                macEV
                        << "[CSMA]: Total number of backoff periods in CAP (except for txBcnDuration): "
                        << t_CAP << endl;
                ...
                macEV
                << "[CSMA]: wtime = " << wtime << "; mpib.getMacBeaconTxTime = "
                        << mpib.getMacBeaconTxTime() << endl;
                tmpf = tmpf / bPeriod;
                macEV
                << "[CSMA]: tmpf = " << tmpf << "; bPeriod  = " << bPeriod
                        << endl;
                t_bPeriods = tmpf - txBcnDuration;
                macEV
                << "[CSMA]: t_bPeriods = " << t_bPeriods << "; txBcnDuration = "
    ...
    } else  // as a device
    {
        if (rxBO != 15)  // beacon enabled as a device
            ...
            if (now >= t_fCAP)  // Check 1
                ...
                bPeriodsLeft = 0;  // re-choose random backoff
                macEV
                        << "[CSMA]: Cannot proceed because it's now NOT in CAP (incoming), resume at the beginning of next CAP \n";
                return false;
            } else {
                t_CAP = (rxSfSpec.finalCap + 1) * rxSfSlotDuration
                        / aUnitBackoffPeriod - rxBcnDuration;
                macEV
                        << "[CSMA]: Total number of backoff periods in CAP (except for rxBcnDuration): "
                        << t_CAP << endl;
                ...
                tmpf -= schedBcnRxTime;
                macEV
                << "[CSMA]: wtime = " << wtime << "; schedBcnRxTime = "
                        << schedBcnRxTime << endl;
                tmpf = tmpf / bPeriod;
                macEV
                << "[CSMA]: tmpf = " << tmpf << "; bPeriod  = " << bPeriod
                ...
                macEV
                << "[CSMA]: t_bPeriods = " << t_bPeriods << "; rxBcnDuration = "
                        << (unsigned short) rxBcnDuration << endl;
    ...
    if (!ok) {
        EV
                  << "[CSMA-CA]:cannot proceed because the chosen number of backoffs cannot finish before the end of current CAP, resume at beginning of next CAP \n";
...
// calculate the time needed to finish the transaction and evaluate it
    ...
    (calcFrameByteLength(tmpCsmaca) <= aMaxSIFSFrameSize) ?
            t_IFS = aMinSIFSPeriod : t_IFS = aMinLIFSPeriod;
    ...
    tmpf += t_transacTime;
    macEV
    << "[CSMA]: The entire transmission is estimated to finish at " << tmpf
            << " sec \n";
...
// evaluate if the entire transmission process can be finished before end of current CAP
    if (tmpf >= t_fCAP) {
        ok = false;
        macEV
                << "[CSMA]: Cannot proceed because the entire transmission cannot finish before the end of current CAP \n";
    } else {
        ok = true;
...
void IEEE802154Mac::csmaca_handle_RX_ON_confirm(phyState status) {
    ...
    if (status != phy_RX_ON) {
        if (status == phy_BUSY_TX) {
            macEV
            << "[CSMA]: Radio is busy TXing now, RX_ON will be set later \n";
...
// locate backoff boundary if needed
    ...
    if (beaconEnabled) {
        macEV
        << "[CSMA]: Locate backoff boundary before sending CCA request \n";
...
//update values
    ...
    if (csmacaWaitNextBeacon) {
        ...
        if (tmpCsmaca && (!backoffTimer->isScheduled())) {
            macEV
            << "[CSMA]: Triggered again by the starting of the new CAP \n";
            ASSERT(bPeriodsLeft >= 0);
            if (bPeriodsLeft == 0)  // backoff again
                    {
                macEV
                        << "[CSMA]: bPeriodsLeft == 0, need to re-choose random number of backoffs \n";
            ...
            } else // resume backoff: this is the second entry to CSMA-CA without calling <csmacaStart()>
            {
                ...
                macEV
                        << "[CSMA]: bPeriodsLeft > 0, continue with the number of backoffs left from last time \n";
...
void IEEE802154Mac::startBackoffTimer(simtime_t wtime) {
    ASSERT(tmpCsmaca != NULL);
    macEV
    << "[CSMA]: #" << (unsigned int) tmpCsmaca->getSqnr()
            << ": starting backoff for " << wtime << " sec \n";
    if (backoffTimer->isScheduled()) {
        cancelEvent(backoffTimer);
    }
...
void IEEE802154Mac::startdeferRxEnableTimer(simtime_t wtime) {
    ...
    macEV
    << "Delay " << wtime << " before starting to work deferred RX-Enable \n";
    return;
}
...
void IEEE802154Mac::startBcnTxTimer(bool txFirstBcn, simtime_t startTime) {
    ...
    if (txFirstBcn)  // tx my first beacon
    {
        if (startTime == 0.0 || isCoordinator == true) // transmit beacon right now for both cases
            ...
            wtime = (aBaseSuperframeDuration
                    * (pow(2, mpib.getMacBeaconOrder()))) / phy_symbolrate;
    ...
    if (!txNow_bcnTxTimer) {
        ...
        wtime = (aBaseSuperframeDuration * (1 << mpib.getMacBeaconOrder()))
                / phy_symbolrate;
        if (bcnTxTimer->isScheduled()) {
            cancelEvent(bcnTxTimer);
        }
        macEV
        << "Scheduling next beacon transmission starts in " << wtime << endl;
        scheduleAt(now + wtime, bcnTxTimer);
    }
...
void IEEE802154Mac::startAckTimeoutTimer() {
    ...
    scheduleAt(simTime() + mpib.getMacAckWaitDuration() / phy_symbolrate,
            ackTimeoutTimer);
}
...
void IEEE802154Mac::startIfsTimer(IFSType ifsType) {
    ...
    wtime = (ifsType == SIFS) ?
            aMinSIFSPeriod / phy_symbolrate : aMinLIFSPeriod / phy_symbolrate;

    macEV
    << "startIfsTimer for IFS Type: " << ((ifsType == SIFS) ? "SIFS" : "LIFS")
            << " with wtime: " << wtime << endl;
...
unsigned char IEEE802154Mac::calcFrameByteLength(cPacket* frame) {
    ...
    if (dynamic_cast<AckFrame*>(frame)) {
        ...
        MHRLength = calcMacHeaderByteLength(
                ((frmFcf & damMask) >> damShift)
                        + ((frmFcf & samMask) >> samShift),
                (bool) ((frmFcf && secuMask) >> secuShift));
        ...
        if (frmType == Beacon) {
            ...
            byteLength = MHRLength
                    + mpduFrm->getEncapsulatedPacket()->getByteLength() + 2; // XXX byteLength of encapsulated MCSP-Data.request
        } else if (frmType == Ack) {
            ...
            if (dynamic_cast<CmdFrame*>(frame)) {
                ...
                cmdFrm = check_and_cast<CmdFrame *>(
            ...
            default: {
                error(
                        "[IEEE802154MAC]: cannot calculate the size of a MAC command frame '%s' with unknown type!",
                        cmdFrm->getName());
                break;
            }
            }  // switch
            ...
            error(
                    "[IEEE802154MAC]: cannot calculate the size of a MPDU frame '%s' with unknown type!",
                    mpduFrm->getName());
        }
    ...
    } else if (dynamic_cast<Association*>(frame)) // indication
    {
        ...
        error(
                "[IEEE802154MAC]: cannot calculate the size of a frame '%s' with unknown type!",
                frame->getName());
    }

    macEV
    << "MAC FrameSize = MHR (" << (int) MHRLength << " Bytew) + Payload ("
            << (int) (byteLength - MHRLength - 2)
            << " Bytes) + MFR (2 Bytes) = " << (int) byteLength << " Bytes \n";

    return byteLength;
}

unsigned char IEEE802154Mac::calcMacHeaderByteLength(unsigned char addrModeSum,
        bool secu) {
    int ashSize = 0;
    if (secu) {
        ashSize = 14;
    ...
    default: {
        error(
                "[IEEE802154MAC]: cannot calculate MAC header byte length due to impossible address mode sum!");
        break;
    }
    }  // switch
...
void IEEE802154Mac::handleBackoffTimer() {
    macEV
            << "[CSMA]: Before sending CCA primitive, tell PHY to turn on the receiver \n";
    taskP.taskStatus(TP_RX_ON_CSMACA) = true;
    genSetTrxState(phy_RX_ON);
    return;
}
...
void IEEE802154Mac::handleBcnTxTimer() {
    ...
    if (mpib.getMacBeaconOrder() < 15) // for Beacon-enabled PANs (beacon order < 0x0F)
        ...
        } else  // it's time to transmit beacon
        {
            ...
            tmpBcn->setFcf(
                    genFCF(Beacon, mpib.getMacSecurityEnabled(), false, false,
                            false, addrLong, 1, addrLong));
            ...
            if (mpib.getMacSecurityEnabled()) {
                ...
                /*
                 std::stringstream madonna;

                 for( int i = 0; i < adata.size(); i++ ) {

                 madonna << std::hex << (0xFF & static_cast<CryptoPP::byte>(adata[i])) << " ";
            ...
            macEV
            << "Sending frame " << txBeacon->getName() << " ("
                    << txBeacon->getByteLength() << " Bytes) to PHY layer \n";
            macEV
            << "The estimated transmission time is " << calcDuration(txBeacon)
                    << " sec \n";
...
void IEEE802154Mac::handleTxAckBoundTimer() {
    if (!beaconWaitingTx) {
        ...
        {
            macEV
            << "Sending " << txAck->getName() << " (" << txAck->getByteLength()
                    << " Bytes | Seq #" << (unsigned int) txAck->getSqnr()
                    << ") to PHY layer \n";
            macEV
            << "The estimated transmission time is " << calcDuration(txAck)
                    << " sec \n";
...
// should turn on radio receiver aTurnaroundTime symbols before GTS starts, if it is a transmit GTS relative to device
    ...
    macEV
    << "[GTS]: Schedule to start GTS index: " << index_gtsTimer << ", slot:#"
            << (int) gtsList[index_gtsTimer].startSlot << " with " << w_time
            << " sec \n";
    startGtsTimer(w_time);
}
...
void IEEE802154Mac::handleGtsTimer() {
    ...
    if (isCoordinator)  // for PAN coordinator
    {
        ...
        macEV
        << "[GTS]: GTS with index = " << (int) indexCurrGts
                << " in my GTS list starts now! \n";
        macEV
        << "allocated for MAC address = "
                << (int) gtsList[indexCurrGts].devShortAddr << ", startSlot = "
                << (int) gtsList[indexCurrGts].startSlot << ", length = "
                << (int) gtsList[indexCurrGts].length << ", isRecvGTS = "
                << gtsList[indexCurrGts].isRecvGTS << ", isTxPending = "
        ...
        if (!gtsList[indexCurrGts].isRecvGTS) {
            macEV
                    << "[GTS]: tell PHY to turn on radio receiver and prepare for receiving packets from device \n";
            genSetTrxState(phy_RX_ON);
        } else    // my time to transmit packets to a certain device
        {
            ...
            if (txGTS && gtsList[indexCurrGts].isTxPending) {
                ...
                macEV
                        << "[GTS]: Data pending for this GTS found in the buffer, starting GTS transmission now \n";
    ...
    } else    // for devices that are not the PAN coordinator
    {
        ...
        if (index_gtsTimer != 99) {
            macEV
            << "[GTS]: my GTS starts now, isRecvGTS = " << isRecvGTS << endl;

            // is receive GTS, turn on radio receiver
            if (isRecvGTS) {
                macEV
                        << "[GTS]: tell PHY to turn on radio receiver and prepare for receiving packets from PAN coordinator \n";
                genSetTrxState(phy_RX_ON);
 ...
 */
unsigned char IEEE802154Mac::gts_request_cmd(unsigned short macShortAddr,
        unsigned char length, bool isReceive) {
    Enter_Method_Silent
    ();
...
// add new GTS descriptor
    ...
    macEV
    << "[GTS]: add a new GTS descriptor with index = " << (int) gtsCount
...
// only be called when my GTS is not the first one in the CFP
    ...
    macEV
            << "[GTS]: it's now the end of CAP, turn off radio and wait for my GTS to start \n";
    genSetTrxState(phy_TRX_OFF);
}
...
void IEEE802154Mac::taskSuccess(char type, bool csmacaRes) {
    if (type == 'b')  //beacon
        ...
        //--- calculate CAP ---
        (txBeacon->getByteLength() <= aMaxSIFSFrameSize) ?
                tmpf += aMinSIFSPeriod : tmpf += aMinLIFSPeriod;
        ...
        unsigned short t_CAP = (txSfSpec.finalCap + 1) * txSfSlotDuration
                / aUnitBackoffPeriod - txBcnDuration;
        ...
        if (t_CAP == 0) // it's possible that there is no time left in current CAP after TXing the beacon
            ...
            csmacaRes = false;
            macEV
                    << "No time left in current CAP after transmitting the beacon, trying to turn off radio \n";
            genSetTrxState(phy_TRX_OFF);
        } else {
        ...
        ASSERT(txData);
        macEV
        << "taskSuccess for sending : " << txData->getName() << ":#"
                << (unsigned int) txData->getSqnr() << endl;
        //Packet *p = txData;
        ...
        ASSERT(txGTS);
        macEV
        << "[GTS]: taskSuccess for sending : " << txGTS->getName() << ":#"
                << (unsigned int) txGTS->getSqnr() << endl;
...
void IEEE802154Mac::taskFailed(char type, MACenum status, bool csmacaRes) {
    ...
    || (type == 'a')  // ACK
        ...
        ASSERT(txData);
        macEV
        << "taskFailed for sending : " << txData->getName() << ":#"
                << (unsigned int) txData->getSqnr() << endl;
        ...
        ASSERT(txGTS);
        macEV
        << "taskFailed for sending : " << txGTS->getName() << ":#"
                << (unsigned int) txGTS->getSqnr() << endl;
        // if PAN coordinator, need to clear isTxPending in corresponding GTS descriptor
        if (isCoordinator) {
            ASSERT(gtsList[indexCurrGts].isTxPending);
...
void IEEE802154Mac::FSM_MCPS_DATA_request(phyState pStatus, MACenum mStatus) {
    ...
    ASSERT(
            ((Ieee802154TxOption ) taskP.mcps_data_request_TxOption >= 1)
                    && ((Ieee802154TxOption ) taskP.mcps_data_request_TxOption
                            <= 3)); // check if TxOption for task pending message is valid
    ...
    if (txOption == DIRECT_TRANS) {
        ...
        case 0: {
            error(
                    "[IEEE802154MAC]: FSM processing MCPS_DATA.request -> something impossible happened here");
            break;
        }
        ...
        case 2: {
            if (((txData->getFcf()) & arequMask) >> arequShift) {
                ...
                macEV
                        << "[MAC-TASK-handleAck]: Data transmitted and timer for ACK is started now \n";
                genSetTrxState(phy_RX_ON);
        ...
        case 3: {
            if (pStatus == phy_SUCCESS)  // ACK received
                    {
                macEV
                << "ACK for " << txData->getName() << ":#"
                        << (unsigned int) txData->getSqnr()
                        << " received successfully \n";
                numRxAckPkt++;
                taskP.taskStatus(task) = false;  // task ends successfully
                macEV
                        << "[MAC-TASK-SUCCESS]: reset TP_MCPS_DATA_REQUEST and cancel AckTimeoutTimer \n";
            ...
            {
                macEV
                << "ACK timeout for " << txData->getName() << ":#"
                        << (unsigned short) txData->getSqnr() << endl;
                numDataRetry++;
                if (numDataRetry <= mpib.getMacMaxFrameRetries()) {
                    macEV
                    << "Starting " << numDataRetry << ". retry for "
                            << txData->getName() << ":#"
                            << (unsigned short) txData->getSqnr() << endl;
                    ...
                    macEV
                    << "The max #num of retries (MaxFrameRetries="
                            << mpib.getMacMaxFrameRetries() << ") reached \n";
        ...
        case 2:  // data successfully transmitted
        {
            ...
            } else  // assume success if ACK not required
            {
                ...
                (txGTS->getByteLength() <= aMaxSIFSFrameSize) ?
                        startIfsTimer(SIFS) : startIfsTimer(LIFS);
            }
            break;
        }  // case 2
        ...
        case 3: {
            if (pStatus == phy_SUCCESS)  // ACK received
                ...
                waitGTSAck = false;
                macEV
                << "[GTS]: ACK for " << txGTS->getName() << ":#"
                        << (unsigned int) txGTS->getSqnr()
            ...
            {
                macEV
                << "[GTS]: ACK timeout for " << txGTS->getName() << ":#"
                        << (unsigned int) txGTS->getSqnr() << endl;
                numGTSRetry++;
                if (numGTSRetry <= mpib.getMacMaxFrameRetries()) {
                    ...
                    macEV
                    << "[GTS]: the #max num of retries (MacMaxFrameRetries="
                            << mpib.getMacMaxFrameRetries()
                            << ") reached, task failed \n";
                    taskP.taskStatus(task) = false;  //task fails
                    macEV << "[MAC-TASK-FAIL]: reset TP_MCPS_DATA_REQUEST \n";
                    waitGTSAck = false;
        ...
        error(
                "[IEEE802154MAC]: the used txOption: %d is not processed -> unknown option",
                txOption);
    }
}

void IEEE802154Mac::dispatch(phyState pStatus, const char *frFunc,
        phyState req_state, MACenum mStatus) {
    /*****************************************/
    /************<csmacaCallBack()>**********/
    /***************************************/
    ...
    /***************************************/
        ...
        // Solution approach: add condition that txBeacon isn't 0
            ...
            {
                (rxCmd->getByteLength() <= aMaxSIFSFrameSize) ?
                        startIfsTimer(SIFS) : startIfsTimer(LIFS);
            ...
            {
                (rxData->getByteLength() <= aMaxSIFSFrameSize) ?
                        startIfsTimer(SIFS) : startIfsTimer(LIFS);
    ...
    /***************************************/
        ...
        if ((txPkt != NULL) && (txPkt == txData)) {
            ...
            macEV
                    << "ACK received but no packet transmitted --> ignore received ACK \n";
        }
    }
    /*****************************************/
    /*******<handleAckTimeoutTimer()>********/
...
void IEEE802154Mac::resetTRX() {
    ...
    if ((mpib.getMacBeaconOrder() != 15) || (rxBO != 15))  // beacon-enabled
        ...
        if ((!inTxSD_txSDTimer) && (!inRxSD_rxSDTimer)) // in inactive portion, go to sleep
                {
            macEV
                    << "Radio is now in inactive period, should turn off radio and go to sleep \n";
            t_state = phy_TRX_OFF;
        ...
        {
            macEV
                    << "Radio is now in incoming active period (as a device), whether go to sleep depending on parameter macRxOnWhenIdle \n";
            t_state = mpib.getMacRxOnWhenIdle() ? phy_RX_ON : phy_TRX_OFF;
        }
    } else  // non-beacon
    {
        macEV
                << "Non-beacon, whether go to sleep depends on parameter macRxOnWhenIdle \n";
        t_state = mpib.getMacRxOnWhenIdle() ? phy_RX_ON : phy_TRX_OFF;
    }
...
IEEE802154Mac::IEEE802154Mac() {
    ...
    for (unsigned char i = 0; i <= 26; i++) {
        ...
        scanPANDescriptorList[i].CoordAddress =
                MACAddressExt::UNSPECIFIED_ADDRESS;
